...
class LaporanRealisasiSumberDanaController extends Controller
{
    public function index(Request $request)
    {
        ...
        $search = $request->get('search', ''); // trim biar tidak ada spasi kosong
        ...
        $sql = "
            ...
            LEFT JOIN 
                ref_sumber_dana d
                ON COALESCE(a.kd_ref1, b.kd_ref1, c.kd_ref1) = d.kd_ref1
        ...
        // hanya filter jika search BENAR-BENAR berisi teks
        if (!empty($search)) {
            $sql .= " AND LOWER(d.nm_ref) LIKE :search ";
            $bindings['search'] = '%' . strtolower($search) . '%';
        }
    
        $sql .= " ORDER BY kd_ref1, kd_ref2, kd_ref3, kd_ref4, kd_ref5, kd_ref6 ";
    
        $data = DB::connection('oracle')->select($sql, $bindings);
